...
function ModalDelivery(props) {
  ...
  const { visible, onCancel, onSuccess, unique_id, id, data, actualLoading } = props;
  ...
  if (!transType || (transType && transType === "sea")) {
    isSEA = true;
    form.setFieldsValue({
      date: moment(actualLoading),
      bl: fileBL && fileBL.url ? fileBL.url : undefined,
    });
  }
  ...
  const handleOnSubmit = (values) => {
    ...
    console.log(values.dateDelivery);
    api.logistic.offers
      .delivery(unique_id, id, moment(values.dateDelivery).format(constant.FORMAT_API_DATE))
  ...
  return (
    <>
      ...
      >
        {console.log(moment(actualLoading))}
        <Form
          form={form}
          onFinish={handleOnSubmit}
          initialValues={{ dateDelivery: actualLoading ? moment(actualLoading) : moment() }}
          layout="vertical"
        >
          {isSEA && (
            <>
