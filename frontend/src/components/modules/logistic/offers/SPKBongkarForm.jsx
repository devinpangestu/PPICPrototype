...
import { Row, Col, Form, Button, Input, InputNumber, Divider, DatePicker } from "antd";
...
const SPKBongkarForm = (props) => {
  const spkType = "pembongkaran";
  ...
  const handleOnSubmit = (values) => {
    ...
    values.unloading_date_start = moment(values.unloading_date[0]).format(constant.FORMAT_API_DATE);
    values.unloading_date_end = moment(values.unloading_date[1]).format(constant.FORMAT_API_DATE);

    api.logistic.spk
      .createBongkar(values)
      .then(function (response) {
        form.resetFields();
      })
      .catch(function (error) {
  ...
  let initialValues = {
    unloading_date_target: Math.ceil(cOffer.quantity / 1000),
  };
  ...
  return (
    <>
      ...
      >
        ...
        <Form.Item
          label={t("Estimated Unloading Date")}
          name="unloading_date"
          rules={[
            {
              required: true,
        ...
        <Form.Item
          label={t("Target Waktu Pembongkaran")}
          name="unloading_date_target"
          rules={[
            {
              required: true,
              message: t("required"),
        ...
        >
          <InputNumber
            placeholder={t("Target Waktu Pembongkaran")}
...
export default withRouter(SPKBongkarForm);
