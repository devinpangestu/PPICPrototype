  ...
  return (
    ...
    >
      <Form labelCol={{ span: 8 }}>
        ...
        {values.type &&
          values.type.toLowerCase() === constant.TRANSPORTIR_TYPE_SEA &&
          values.ship && (
        ...
        <Form.Item label={`${t("Estimated")} ${t("loadingDate")}`} className="mb-0">
          {values.loading_date && <Input value={moment(values.loading_date)} bordered={false} />}
        </Form.Item>
        <Form.Item label={t("etaWarehouse")} className="mb-0">
          {values.eta_warehouse && <Input value={moment(values.eta_warehouse)} bordered={false} />}
        </Form.Item>
      </Form>
    </Modal>
  );
