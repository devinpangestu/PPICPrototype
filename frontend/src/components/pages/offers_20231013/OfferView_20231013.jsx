...
const OfferView = (props) => {
  ...
  const loadOffers = async () => {
    ...
    setPageLoading(true);
    const otherParams = {
      from_date: dateRange[0].format(constant.FORMAT_API_DATE),
      to_date: dateRange[1].format(constant.FORMAT_API_DATE),
    };
  ...
  const handleEdit = (offerId) => {
    Modal.confirm({
      ...
      onOk: () => {
        props.history.push(`/offers_20231013/${offerId}/edit`);
      },
    });
  };
    ...
    const renderTransportirAction = (offer) => {
      ...
      return (
        <Button
          type="primary"
          onClick={() => {
            utils.openInNewTab(`/logistic/offers_20231013/create?commodity_offer=${offer.id}`);
          }}
        >
          {t("manageLogistic")}
        </Button>
  ...
  return (
    <PageContainer
      ...
      additionalAction={utils.renderWithPermission(
        userInfo.permissions,
        <Link to="/offers_20231013/create">
    ...
    >
      ...
      <ModalPriceData
        ...
        date={selectedOffer && selectedOffer.datetime ? selectedOffer.datetime : new Date()}
        setPageLoading={setPageLoading}
      />
